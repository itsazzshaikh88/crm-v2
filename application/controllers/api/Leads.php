...
class Leads extends Api_controller
{
            ...
            if ($created) {
                ...
                if ($consultant_details) {
                    $emailViewConfig = [
                        ...
                        'heading' => "New Lead Assigned: " . $newlyCreatedLead['COMPANY_NAME'],
                    ];
                    $leadDetailsForEmailContent = [
                        'user' => "$consultant_details[FIRST_NAME] $consultant_details[LAST_NAME]",
                        'lead_name' => "$newlyCreatedLead[FIRST_NAME] $newlyCreatedLead[LAST_NAME]",
                        'company' => $newlyCreatedLead['COMPANY_NAME'],
                        'email' => $newlyCreatedLead['EMAIL'],
                        'phone' => $newlyCreatedLead['PHONE'],
                        'assigned_on' => date('d-M-Y'),
                        'link' => base_url("leads?u_source=email&mode=lead-assigned&lead-id=" . $newlyCreatedLead['LEAD_ID'])
                    ];
                    $mailContent = $this->load->view('email-templates/layout', ['emailViewConfig' => $emailViewConfig, 'leadDetails' => $leadDetailsForEmailContent], true);

                    $consultant_email_address = $consultant_details['EMAIL'] ?? 'IT@zamilplastic.com';

                    $this->app_mailer([
                        'to'        => $consultant_email_address,
                        'subject'   => "New Lead Assigned: " . $newlyCreatedLead['COMPANY_NAME'],
                        'message'   => $mailContent,
    ...
    // Modifications based on the new 
    function new_lead()
    {
        // Check if the authentication is valid
        $isAuthorized = $this->isAuthorized();
        ...
        try {
            ...
            $this->form_validation->set_rules('COMPANY_NAME', 'Company Name', 'required');
            $this->form_validation->set_rules('LEAD_SOURCE', 'Lead Source', 'required');
            $this->form_validation->set_rules('LEAD_EVENT', 'Lead Event', 'required');
            $this->form_validation->set_rules('ASSIGNED_TO', 'Sales Person Name', 'required');
            $this->form_validation->set_rules('ORG_ID', 'Org ID', 'required');
            ...
            // Save Data to the product table
            $newlyCreatedLead = $this->Lead_model->add_lead_details($data, $isAuthorized['userid']);
    ...
    function update_lead($leadID)
    {
        ...
        try {
            ...
            // Check if lead details are present in table with existing ID
            $lead = $this->Lead_model->get_lead_details_by_id($leadID ?? 0);
            if (empty($lead)) {
                $this->sendHTTPResponse(404, [
                    'status' => 'error',
                    'code' => 404,
            ...
            // Save Data to the lead table
            $updatedLead = $this->Lead_model->update_lead_details($leadID, $data, $isAuthorized['userid']);
    ...
    public function lead_detail($leadID)
    {
        // Check if the authentication is valid
        $isAuthorized = $this->isAuthorized();
        if (!$isAuthorized['status']) {
        ...
        $lead = $this->Lead_model->get_lead_details_and_activities_by_id($leadID);
