...
class Sales extends Api_controller
{
    ...
    function salespersons()
    {
        ...
        $total_salespersons = $this->Sales_model->get_salespersons('total', $limit, $currentPage, $filters, $search);
        $salesPersons = $this->Sales_model->get_salespersons('list', $limit, $currentPage, $filters, $search);

        $response = [
            'pagination' => [
                'total_records' => $total_salespersons,
                'total_pages' => generatePages($total_salespersons, $limit),
                'current_page' => $currentPage,
                'limit' => $limit
            ],
    ...
    function add_sales_person()
    {
        ...
        try {
            ...
            $fields = ['FIRST_NAME', 'LAST_NAME', 'EMAIL', 'PHONE', 'EMPLOYEE_ID', 'DEPARTMENT', 'DESIGNATION', 'DATE_OF_JOINING', 'CRM_ROLE', 'LOGIN_ACCESS', 'STATUS'];
            ...
            // Save Data to the product table
            $newlyCreatedSalesPerson = $this->Sales_model->add_sales_person($data, $isAuthorized['userid']);

            if ($newlyCreatedSalesPerson) {
                ...
                $action = [
                    'ACTIVITY_TYPE' => "SALES PERSON {$action_type}",
                    'DESCRIPTION' => "User {$userForActivity['name']} (Role: {$userForActivity['role']}) {$action_type} sales person account from IP {$system['IP_ADDRESS']} using {$system['BROWSER']} on " . date('D, d M Y - H:i:s')
                ];
                ...
                $this->sendHTTPResponse(201, [
                    ...
                    'type' => 'insert',
                    'data' => $newlyCreatedSalesPerson,
                ]);
            } else {
                throw new Exception('Failed to create new sales person.');
            }
        } catch (Exception $e) {
            // Catch any unexpected errors and respond with a standardized error
            $this->sendHTTPResponse(500, [
    ...
    function update_sales_person($salesPersonID)
    {
        // Check if the authentication is valid
        $isAuthorized = $this->isAuthorized();
        if (!$isAuthorized['status']) {
        ...
        try {
            ...
            // Check if Forecast details are present in table with existing ID
            $salesPerson = $this->Sales_model->get_sales_person_by_id($salesPersonID ?? 0);
            if (empty($salesPerson)) {
                $this->sendHTTPResponse(409, [
                    'status' => 'error',
                    'code' => 404,
                    'error' => 'Not Found',
                    'message' => 'A sales person with provided Id does not found to update.'
                ]);
                return;
            }

            // Save Data to the Forecast table
            $updatedSalesPerson = $this->Sales_model->update_sales_person($salesPersonID, $data, $isAuthorized['userid']);
            ...
            if ($updatedSalesPerson) {
                ...
                $action = [
                    'ACTIVITY_TYPE' => "SALES PERSON {$action_type}",
                    'DESCRIPTION' => "User {$userForActivity['name']} (Role: {$userForActivity['role']}) {$action_type} sales person from IP {$system['IP_ADDRESS']} using {$system['BROWSER']} on " . date('D, d M Y - H:i:s')
                ];
    ...
    public function sales_person_detail($salesPersonID)
    {
        // Check if the authentication is valid
        $isAuthorized = $this->isAuthorized();
        if (!$isAuthorized['status']) {
        ...
        // Validate input and check if `productUUID` is provided
        if (!$salesPersonID || !isset($salesPersonID)) {
            return $this->output
                ->set_status_header(400)
                ->set_content_type('application/json')
                ->set_output(json_encode([
        ...
        $salesPerson = $this->Sales_model->get_sales_person_by_id($salesPersonID);
        ...
        if (empty($salesPerson)) {
            return $this->output
                ...
                ->set_output(json_encode([
                    'status' => 'error',
                    'code' => 404,
                    'message' => 'Sales person details not found.'
                ]));
        }
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                'status' => 'success',
                'code' => 200,
                'message' => 'Sales person details retrieved successfully',
                'data' => $salesPerson
            ]));
    }

    function delete_salesperson($salesPersonID)
    {
        // Check if the authentication is valid
        $isAuthorized = $this->isAuthorized();
        if (!$isAuthorized['status']) {
        ...
        if (empty($salesPersonID) || !is_numeric($salesPersonID)) {
            $this->output
                ->set_content_type('application/json')
                ->set_status_header(400) // 400 Bad Request status code
                ->set_output(json_encode(['error' => 'Invalid Sales Person ID.']));
            return;
        }

        // Attempt to delete the Forecast
        $result = $this->Sales_model->delete_sales_person_by_id($salesPersonID);
        if ($result) {
