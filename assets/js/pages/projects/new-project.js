...
async function submitForm(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            toasterNotification({ type: 'success', message: "Project Saved Successfully!" });
            if (data?.type === 'insert') {
                form.reset();
...
async function fetchProjectDetails(projectUUID) {
    const apiUrl = `${APIUrl}/projects/detail`;
    const authToken = getCookie('auth_token');
    if (!authToken) {
        toasterNotification({
            type: 'error',
    ...
    try {
        ...
        if (data?.data?.ATTACHMENTS) {
            ...
            displayUploadedFiles(data?.data?.PROJECT_ID || 0);
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const projectUUID = urlSegments[urlSegments.length - 1];
