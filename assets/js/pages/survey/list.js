...
async function fetctSurvey(userSearchTerm = null) {
	if (!userSearchTerm) {
		userSearchTerm =
			document.getElementById("searchInputElement").value.trim() || null;
	}
	try {
...
function showSurvey(surveys, tbody) {
	...
	if (surveys?.length > 0) {
		...
		surveys.forEach((survey, index) => {
			...
			if (img != null) img = `${PRODUCT_IMAGES_URL}${img}`;

			content += `<tr data-survey-id="${survey.SURVEY_ID}">
                            <td>${index + 1}</td>
                            <td class=" ">${survey.SURVEY_NAME}</td>
                            <td class=" ">${survey.SURVEY_DESC || ""}</td>
                            <td class="">${formatAppDate(
															survey.START_DATE || ""
														)} - ${formatAppDate(survey.END_DATE || "")}</td>
                            <td class="">${survey.CONDUCTED_BY || ""}</td>
                            <td class="">
                                <span class="text-white badge" style="background-color: ${
																	surveyStatusColors[survey.STATUS || ""]
																}">
                            ...
                            <td class="text-center">
                                <div class="d-flex align-items-center justify-content-center gap-3">
                                    ...
                                    <a title="Edit Survey" href="survey/new/${
																			survey.UUID
																		}?action=edit">
                                        <small>
			...
			if (isAdmin) {
				content += `<a title="Delete Survey" href="javascript:void(0)" onclick="deleteSurvey(${survey.SURVEY_ID})">
                                        <small>
