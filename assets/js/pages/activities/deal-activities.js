...
const activityTypes = {
    ...
    'task': 'custom-activity-modal-task',
    'event': 'custom-activity-modal-event'
}

function openActivityModal(modalID) {
    currentOpenedModal = modalID
    // Assign lead ID to the modal
    document.getElementById(`${modalID}-lbl-LEAD_NUMBER`).value = document.getElementById("DEAL_NUMBER").value || ''
    document.getElementById(`${modalID}-ACTIVITY_UUID`).value = uuid_v4()
    document.getElementById(`${modalID}-ACTIVITY_LEAD_ID`).value = document.getElementById("DEAL_ID").value || ''
    // Show Modal 
...
async function addLeadActivity(e) {
    ...
    formData.append("ACTIVITY_SOURCE", 'deals');
...
function setActivitySaved(data, action) {
    ...
    } else if (data?.activity?.ACTIVITY_TYPE?.toLowerCase() === 'task') {
        activityContent = showNewTaskActivity(data || {}, action)
    }
...
function showNewMeetingActivity(activityData, action) {
    ...
    let content = `<div class="position-relative ps-6 pe-3 py-2 bg-gray-50s" id="activity-inline-container-${activity?.ACTIVITY_ID}">
                                    ...
                                    <div class="mt-2">
                                        <div>
                                            <p for="" class="mb-0 fw-bold">Agenda:<span class="text-gray-800 fw-normal">${details?.AGENDA}</span></p>
                                            <p for="" class="mb-0 fw-bold">Location:<span class="text-gray-800 fw-normal">${details?.LOCATION}</span></p>
...
function showNewTaskActivity(activityData) {
    if (!activityData) return '';
    const { activity, details } = activityData;
    return `<div class="position-relative ps-6 pe-3 py-2 bg-gray-50s" id="activity-inline-container-${activity?.ACTIVITY_ID}">
                                    ...
                                    <div class="mt-2">
                                        <div>
                                            <p for="" class="mb-0 fw-bold">Due Date:<span class="text-gray-800 fw-normal"> ${formatAppDate(details?.DUE_DATE)}</span></p>
                                            <p for="" class="mb-0 fw-bold">Priority:<span class="text-gray-800 fw-normal"> ${details?.PRIORITY}</span></p>
                                            <p for="" class="mb-0 fw-bold">Task Description:</p>
                                            <p class="line-clamp-2 text-gray-800 mb-0">${stripHtmlTags(details?.TASK_DESCRIPTION)}</p>
                                        </div>
                                    </div>
                                </div>
...
function displayActivityData(activityData, modalID) {
    ...
    if (activity?.ACTIVITY_TYPE?.toLowerCase() === 'call')
        ...
        notes = details?.TASK_DESCRIPTION;
    // Initialize Quill editor
...
async function deleteCurrentActivity(activityID) {
    ...
    try {
        ...
        const response = await fetch(`${APIUrl}/activities/delete/${activityID}/deals`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${authToken}`,
                'Content-Type': 'application/json'
