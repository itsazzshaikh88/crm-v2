		...
		complaints.forEach((complaint) => {
			...
			content += `<tr data-complaint-id="${
				complaint.COMPLAINT_ID
							...
							<td class=""> <small class="text-white bg-${color} border border-${color} px-2 py-1 rounded">${
				complaint.STATUS || ""
			}</small></td>
							<td class="text-end">
								<div class="d-flex align-items-center justify-content-end gap-3">
									${
										(complaint.STATUS === "Draft" && userType === "client") ||
										(complaint.STATUS === "Active" && userType === "client")
											? `
											<a title="Edit Complaint" href="complaints/new/${complaint.UUID}?action=edit">
												<small>
													<i class="fs-8 fa-regular fa-pen-to-square text-gray-700"></i>
												</small>
											</a>
											<a title="Delete Complaint" href="javascript:void(0)" onclick="deleteRequest(${complaint.COMPLAINT_ID})">
												<small>
									...
									${
										(complaint.STATUS === "Draft" && userType === "admin") ||
										(complaint.STATUS === "Active" && userType === "admin")
											? `
											<a title="Resolve Complaint" href="complaints/resolve/${complaint.COMPLAINT_ID}">
												<small>
				...
				<a title="View Complaint" href="complaints/view/${complaint.UUID}">
										<small>
	...
	try {
		...
		if (data.status) {
			...
			const row = document.querySelector(
				`#complaint-list-tbody tr[data-complaint-id="${complaintID}"]`
			);
			if (row) {
