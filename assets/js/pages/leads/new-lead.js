    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data?.type == 'insert') {
                ...
                toasterNotification({ type: 'success', message: "Lead Created Successfully" });
            } else if (data?.type == 'update') {
                // Data is updated
                toasterNotification({ type: 'success', message: "Lead Updated Successfully" });
            } else {
...
async function fetchLead(leadID) {
    const apiUrl = `${APIUrl}/leads/detail`;
    const authToken = getCookie('auth_token');
    if (!authToken) {
        toasterNotification({
            type: 'error',
...
function textInputElementLoadingAnimation(type = 'set') {
    const formElements = document.querySelectorAll(".lead-form-elements .form-control");
    if (type === 'set') {
