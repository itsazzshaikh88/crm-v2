...
async function fetchSalesPersonListFromModal(userSearchTerm = null) {
	try {
		...
		clientListModalSkeleton(
			listContainer,
			salesPersonModalListPaginate.pageLimit || 0
		);
		...
		const response = await fetch(url, {
			...
			body: JSON.stringify({
				...
				filters: { STATUS: "active", ROLE_NAME: "Sales Person" },
		...
		salesPersonModalListPaginate.totalPages =
			parseFloat(data?.pagination?.total_pages) || 0;
		salesPersonModalListPaginate.totalRecords =
...
const salesPersonModalListPaginate = new Pagination(
	"spml-current-page",
	"spml-total-pages",
	"spml-page-of-pages",
	"spml-range-of-records"
);
...
// Modal Related Code
var salesPersonListModal = new bootstrap.Modal(
	document.getElementById("sales-person-list-modal"),
	{
...
const debouncedSearchSalesPersonListFromModal = debounce(
	searchSalesPersonListFromModal,
	300
); // 300ms delay
